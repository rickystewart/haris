...
haris_int16_t haris_read_int16(unsig",
"ned char *);\n\
...
void haris_write_uint32(unsigned",
" char *, haris_uint32_t);\n\
...
{\n\
  if (*b & 0x80)\n\
    return -((haris_int32_t)(*b & 0x7F) << 24 | (haris_int32_t)*(b+1) << 16 |\n\
             (haris_i",
"nt32_t)(*b+2) << 8 | (haris_int32_t)(*b+3)) - 1;\n\
  else\n\
...
"\n\
\n\
haris_float32 haris_read_float32(unsigned char *b)\n\
{\n\
...
{\n\
  ...
  result += 1.0;\n\
\n\
  shift = ((i >> HARIS_FLOAT64_SIGBITS) & 2047) - HARIS_FLOAT64_B",
"IAS;\n\
  while (shift > 0) { result *= 2.0; shift--; }\n\
  while (shift < 0) { result /= 2.0; shift++; }\n\
\n\
...
\n\
void haris_write_float32(unsigned char *b, haris_float32 f)\n\
{\n\
  haris_float64 fnorm;\n\
  int shift;\n\
...
\n\
  ...
  fnorm = fnorm - 1.0;\n\
\n\
  significand = fnorm * ((1LL<<HARIS_FLOAT64_SIGBITS)",
" + 0.5f);\n\
\n\
  exp = shift + ((1<<10) - 1); \n\
\n\
