...
static CJobStatus write_util_h_top(CJob *job);
static CJobStatus write_util_h_typedefs(CJob *job);
static CJobStatus write_util_h_defines(CJob *job);
static CJobStatus write_util_h_bottom(CJob *job);
...
CJobStatus write_utility_library_header(CJob *job)
{
  CJobStatus result;
  if ((result = write_util_h_top(job)) != CJOB_SUCCESS ||
      (result = write_util_h_typedefs(job)) != CJOB_SUCCESS ||
      (result = write_util_h_defines(job)) != CJOB_SUCCESS ||
      (result = write_util_h_bottom(job)) != CJOB_SUCCESS)
    return result;
  return CJOB_SUCCESS;
}

CJobStatus write_utility_library_source(CJob *job)
{
  CJobStatus result;
  if ((result = write_util_c_top(job)) != CJOB_SUCCESS ||
      (result = write_util_c_readint(job)) != CJOB_SUCCESS ||
      (result = write_util_c_readuint(job)) != CJOB_SUCCESS ||
      (result = write_util_c_writeint(job)) != CJOB_SUCCESS ||
      (result = write_util_c_writeuint(job)) != CJOB_SUCCESS ||
      (result = write_util_c_readfloat(job)) != CJOB_SUCCESS ||
      (result = write_util_c_writefloat(job)) != CJOB_SUCCESS)
    return result;
  return CJOB_SUCCESS; 
}
...
static CJobStatus write_util_h_top(CJob *job)
{
  CJOB_FMT_HEADER_STRING(job, "%s%s%s%s", "#ifndef __HARIS_UTIL_H\n\
